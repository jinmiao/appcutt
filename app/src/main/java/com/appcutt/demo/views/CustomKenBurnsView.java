...
public class CustomKenBurnsView extends KenBurnsView {
    ...
    public CustomKenBurnsView(Context context, AttributeSet attrs) {
        ...
        TypedArray a = context.obtainStyledAttributes(attrs, com.appcutt.demo.R.styleable.AspectRatioImageView);
        try {
            ratio = a.getFloat(com.appcutt.demo.R.styleable.AspectRatioImageView_imageRatio, DEFAULT_RATIO);
            autoScale = a.getBoolean(com.appcutt.demo.R.styleable.AspectRatioImageView_autoScale, false);
        } finally {
            a.recycle();
        }
    }
